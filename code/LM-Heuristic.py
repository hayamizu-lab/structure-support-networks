...
# --------------------------------------
# heuristic algorithm for solving LEVEL MINIMIZATION (Algorithm 2)
def heuristic_algorithm_min_lev(network):
    # enumerate the family of minimum support networks C_N
    support_networks = enumerate_all_minimum_support_networks(network)
...
heuristic_min_lev, min_lev_network = heuristic_algorithm_min_lev(network)
end_time = time.perf_counter()
runtime = end_time - start_time

# output
print(f"Input: {filename}.txt")
print(f"Minimum level:{heuristic_min_lev}")
print(f"Runtime (sec):{runtime}")
draw_subnetwork(network, min_lev_network, filename + "-heuristic")